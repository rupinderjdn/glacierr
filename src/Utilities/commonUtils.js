...
export const getScreenWidgetProperties = (screenId,widgetId,configId)=>{
  return store.getState().UILayoutData.ScreenChildrenWidgetsProperties[screenId][widgetId][configId];
}
    ...
    ticketFields.forEach(ticketField => {
      if(agGridHashMap[ticketField.dataIndex] !== true){
        let agGridConfigObj = {
          ...
          type: ticketField.rendererTypes.length>0?ticketField.rendererTypes:ticketField.dataType
        }
