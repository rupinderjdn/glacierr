...
import { CITIES_TAB, SETTINGS_TAB, WEATHER_TAB } from "../../Store/storeConstants";
...
const SideMenuView = (props) => {
  ...
  return (
    ...
    >
      ...
      <div onClick={()=>tabSelected(WEATHER_TAB)} className="flex my-4 px-2 flex-col items-center hover:text-gray-400 cursor-pointer transition-colors md:my-6">
        <TiWeatherWindyCloudy className="px-1 mb-2 text-3xl md:text-4xl text-center" />
        <div className="text-sm md:text-base">Weather</div>
      </div>
      <div onClick={()=>tabSelected(CITIES_TAB)} className="my-4 px-2 flex flex-col items-center hover:text-gray-400 cursor-pointer transition-colors md:my-6">
        <BsMenuButtonWide className="px-1 mb-2 text-2xl md:text-3xl text-center" />
        <div className="text-sm md:text-base">Cities</div>
      </div>
      <div onClick={()=>tabSelected(SETTINGS_TAB)} className="my-4 px-2 flex flex-col items-center hover:text-gray-400 cursor-pointer transition-colors md:my-6">
